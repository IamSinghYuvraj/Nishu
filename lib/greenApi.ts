...
class GreenApiClient {
  ...
  async sendMessage(phoneNumber: string, message: string): Promise<ApiResponse<SendMessageResponse>> {
    try {
      ...
      if (!this.config.idInstance || !this.config.apiTokenInstance) {
        return {
          success: false,
          error: 'Green API credentials are not configured. Please set GREEN_API_ID_INSTANCE and GREEN_API_TOKEN_INSTANCE in .env.local',
        };
      }

      const chatId = this.formatPhoneNumber(phoneNumber);
      const apiUrl = this.getApiUrl('sendMessage');
...
if (typeof window === 'undefined') {
  ...
  console.log('Green API Client Initialized:', {
    hasIdInstance: !!process.env.GREEN_API_ID_INSTANCE,
    hasTokenInstance: !!process.env.GREEN_API_TOKEN_INSTANCE,
    baseUrl: process.env.GREEN_API_BASE_URL || 'https://api.green-api.com',
  });
}
